...
public class PlayerController : MonoBehaviour
{
    ...
    // Bullet Directions for this example
    private enum BulletDirections { Case1, Case2, Case3 };
    [SerializeField] BulletDirections bulletDirection = BulletDirections.Case1;
    ...
    void ShootBullet()
    {
        ...
        {
            case BulletDirections.Case1:
                /*
                 * CASE 1 - horizontal only
                 */
                // fire bullet horizontal facing direction
                bullet.GetComponent<BulletScript>().SetBulletDirection((isFacingRight) ? Vector2.right : Vector2.left);
                break;
            case BulletDirections.Case2:
                /*
                 * CASE 2 - vertical OR horizontal
                 */
                if (keyVertical > 0)
                {
                    // fire bullet upward
                    bullet.GetComponent<BulletScript>().SetBulletDirection(Vector2.up);
                }
                else if (keyVertical < 0)
                {
                    // fire bullet downward
                    bullet.GetComponent<BulletScript>().SetBulletDirection(Vector2.down);
                }
                else
                {
                    // fire bullet horizontal facing direction
                ...
                /* 
                 * OR like Case 3 below but just set the X velocity to zero
                 */
                /*
                if (keyVertical != 0)
                {
                    bullet.GetComponent<BulletScript>().SetBulletDirection(new Vector2(0, keyVertical));
                }
                else
                {
                    bullet.GetComponent<BulletScript>().SetBulletDirection((isFacingRight) ? Vector2.right : Vector2.left);
                ...
                */
                break;
            case BulletDirections.Case3:
                /*
                 * CASE 3 - vertical AND horizontal
                 */
                if (keyVertical != 0)
                {
                    // fire bullet vertically with horizontal movement (if applied)
                    // use keyboard input values as vectors since we know Input.GetAxisRaw()
                    // return values are -1f, 0, 1f (left/down, no input, right/up)
                    bullet.GetComponent<BulletScript>().SetBulletDirection(new Vector2(keyHorizontal, keyVertical));
                }
                else
                {
                    // fire bullet horizontal facing direction (no vertical input)
                    bullet.GetComponent<BulletScript>().SetBulletDirection((isFacingRight) ? Vector2.right : Vector2.left);
                }
                break;
        }

        // finally shoot the bullet
