...
public class BulletScript : MonoBehaviour
{
    ...
    public enum BulletTypes { Bullet1, Bullet2, Bullet3, Bullet4, Bullet5, Bullet6, Axe, Fireball };
    [SerializeField] BulletTypes bulletType = BulletTypes.Bullet1;
    ...
    {
        // set animator, sprite image & collider radius
        animator.runtimeAnimatorController = bulletData[(int)type].animator;
        sprite.sprite = bulletData[(int)type].sprite;
        cc2d.offset = bulletData[(int)type].offset;
    ...
    public void Shoot()
    {
        // assumed right facing bullets sprites and animations
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        foreach (string tag in collideWithTags)
        {
            ...
            if (other.gameObject.CompareTag(tag))
            {
                ...
                {
                    case "Enemy":
                        // collide with an object tagged Enemy
                        break;
                    case "Player":
                        // collide with an object tagged Player
                        break;
                }
